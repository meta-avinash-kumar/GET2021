...
SELECT p.product_name, p.product_price, p.product_description from products p join subcategory sc where sc.subcategory_name = 'mobile';
...
SELECT order_id, order_date_time, order_amount from orders where order_status = 'not Shipped' and CURDATE()-order_date_time>=10;

SELECT c.customer_id, c.customer_name from customer c join orders o ON c.customer_id=o.customer_id where c.customer_id NOT IN
(SELECT c.customer_id from customer c join orders o where c.customer_id=o.customer_id and CURDATE()-o.order_date_time<=31);

SELECT c.customer_id, c.customer_name from customer c join orders o ON c.customer_id=o.customer_id where CURDATE()-o.order_date_time<15;
